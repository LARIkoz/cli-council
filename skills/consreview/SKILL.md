---
name: consreview
description: Convene a multi-model CODE-REVIEW council on the cli-council engine (`council review`) — several independent subscription-CLI models review a diff / PR / files, anonymously peer-rank, a chairman synthesizes ONE severity-classified review, then independent audit + redteam panels gate it (worst-wins — a rule, not a chairman). Auto-use when the user wants MORE than a single-model glance — a multi-voice or second-opinion review of a code change, a hard review before shipping / committing / merging, or names "consreview" / "council review" or says "прогони ревью советом", "ревью-совет", "консревью", "разбери диф несколькими моделями", "жёсткое ревью перед мержем". Each run spends a few wall-minutes across the CLIs you already authed ($0, no API keys) — reserve it for changes worth that, not a one-line diff (for those, just review it directly).
user-invocable: true
---

Convene a code-review **council** on the **cli-council** engine (`council review`).

**Input.** If this ran as `/consreview <ref|files>`, that argument is the target. If it
auto-triggered from the conversation, determine the target yourself from context (the
change under discussion — current diff, a git ref, specific files, or pasted code). If
the change is trivial (a one-line diff, a comment tweak), do NOT convene the council —
just review it directly. A run is $0 via the user's subscriptions but burns a few minutes.
...
2. **Determine target** (one `council review` input form):
   - Empty / current changes → `council review` (uncommitted `git diff HEAD`)
...
3. **Run the review** (background it — a few minutes with slow reasoning voices + 2 panels):
   ...
   - **`--no-redteam`** requested → append `--redteam ' '` (a whitespace-only list =
...
- `commands/consreview.md` — the explicit slash-command form of this exact procedure.
