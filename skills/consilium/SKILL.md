...
Convene a decision/opinion **council** on the **cli-council** engine (`council decide`).

**Input.** If this ran as `/consilium <decision>`, that argument is the decision. If it
auto-triggered from the conversation, extract the decision to weigh yourself — the forks,
the constraints, and what a good answer must address. If the decision is empty or too
vague to frame, ask ONE clarifying question before firing (a run is $0 via the user's
subscriptions but burns a few minutes of wall time). If the request is a trivial or
quick-fact question, do NOT convene the council — just answer.
...
3. **Run the decision council** (background it — a few minutes with the slow reasoning
   ...
   - **`--redteam`** requested → append `--redteam <v1,v2>` (a lean adversarial panel
